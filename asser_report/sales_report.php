...
$max=$_GET["fechamax"];
$query1= $db->GetAll("
select v.idturno,s.idsucursal, 
sum(v.total)as total, s.nombre as sucursal, 
COUNT(v.nro)as tt
from venta v, sucursal s
where v.pago != 'comida_personal' AND v.estado='V' and v.sucursal_id=s.idsucursal and v.fecha
...
from venta v, sucursal s
where v.pago != 'comida_personal' AND v.estado='V' and sucursal_id=$s and turno=$t and v.sucursal_id=s.idsucursal and v.fecha
...
<tr class=warning>
    ...
    <td><?php $n2=2; $turno_pm=number_format(pm_t($r["idsucursal"],'2',$min,$max),2);  $pm_idturno=pm_t_i($r["idsucursal"],2,$min,$max);
