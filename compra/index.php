...
$total_bodega = 0;
$total_sucursales = 0;
$totalbodega = $db-> GetAll("SELECT sum(total) as totalbodega from compra c 
...
WHERE c.fecha BETWEEN '$min' AND '$max' and c.sucursal_id = 1 ");
$totalsucursales = $db-> GetAll("SELECT sum(total) as totalsucursales from compra c 
INNER JOIN usuario u ON u.idusuario = c.usuario_id  
INNER JOIN proveedor pro ON c.proveedor_id = pro.idproveedor
INNER JOIN sucursal s ON s.idsucursal = c.sucursal_id

WHERE c.fecha BETWEEN '$min' AND '$max' and c.sucursal_id BETWEEN 
2 and 16;");

foreach($totalbodega as $total){
  $total_bodega= $total['totalbodega'];
}

foreach($totalsucursales as $totalsuc){
  $total_sucursales= $totalsuc['totalsucursales'];
}
...
<?php $nro=0; $total=0; $total2=0; $suma_pago = 0;
  foreach ($query as $r){
    ...
    $pagos_realizados = $db->Execute("SELECT SUM(monto) as monto FROM pago p WHERE p.compra_id = '$compra_id'"); //sacar los montos pagados
    foreach($pagos_realizados as $i){
      $pago      = $i['monto'];
...
<trclass=warning>
  ...
  <td><?php echo $r["proveedor"]; ?></td>
    <?php if(number_format($r["total"],2) == number_format($pago,2)) {?>
      <td style="color: green">Pagada</td>
    <?php }else{?>
      <td style="color: red;"><?php echo $r["estado"]; ?></td>
    <?php }?>
  <td><?php echo number_format($pago,2);  ?></td>
  <td><?php echo number_format($deuda,2); ?></td>
  <td><?php echo number_format($r["total"],2)."bs"; ?></td>
  <td style="width:210px;">
    <a href="#" data-toggle="modal"
                  data-target="#ver<?php echo $r["nro"];?>"><img src="../images/ver.png" alt="" title="ver detalle">Detalle</a>
    <?php
    ...
    echo "&nbsp; <a href='../pago/index.php?nro=$r[nro]' class='' role='button'><img src='../images/pago.jpg' alt=''>Pagos</a>";
    ?>
  </td>
  </tr>
...
<?php }?>
  ...
  <table id="usuario" class="table table-striped table-hover table-bordered" cellspacing="0" width="100%">
  <tr>
  <td><h4 class="totales">Total de Pagos realizados</h4></td><td><h4 class="totales"> <?php echo  number_format($suma_pago,2) ." bs"; ?></h4><td><h4 class="totales">Total Bodega</h4><td><h4 class="totales"> <?php echo number_format($total_bodega,2)." bs"; ?></h4></td><td><h4 class="totales">Total Sucursales</h4></td><td><h4 class="totales"> <?php echo number_format($total_sucursales,2)." bs"; ?></h4></td><td><h4 >Total Compras de Insumos</h4></td><td><h4 class="totales"> <?php echo number_format($total,2)." bs"; ?></h4>
  </td></tr>
  </table>
  </div>
  </div>
