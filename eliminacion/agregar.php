...
$obs=$_POST['obs'];
$hora=date("H:i:s");
$total_eliminacion = $db->GetOne("select sum(subtotal) from detalleeliminacion
							 where nro = '$nro' and sucursal_id='$sucur'");

$inventario = $db->GetOne("SELECT max(nro) FROM inventario where sucursal_id=".$sucur);

$insertar = $db->Execute("insert into eliminacion(nro,hora,fecha,total,descripcion,usuario_id,sucursal_id,inventario_id)
