...
$nro = $_GET['nro'];	
	$sql = $db ->GetRow("select p.*,c.*,date_format(c.fecha,'%d-%m-%Y') as fecha from provedor p,orden_de_compra c 
...
$pdf->Ln(5);
$pdf->setFont('arial','',10);
$dv = $db ->GetAll("select p.*,dc.* from detalle_orden_de_compa dc,producto p
