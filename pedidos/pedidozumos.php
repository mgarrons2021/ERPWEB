...
(select s.idsucursal,p.idproducto,p.nombre as produccion, sum(dt.cantidad)as cantidad , t.hora_solicitud as hora
from detallepedido dt,pedido t, sucursal s,producto p
where p.idcategoria=10 and p.idproducto=dt.producto_id
and t.sucursal_id=dt.sucursal_id and dt.nro=t.nro
and t.sucursal_id=s.idsucursal and t.fecha_p between '$min'
and '$max' and dt.sucursal_id BETWEEN 2 and 16 GROUP BY p.idproducto order by t.hora_solicitud asc);");
...
from detallepedido dt,pedido t, sucursal s,producto p
where p.idproducto=dt.producto_id and dt.producto_id = p.idproducto
and t.sucursal_id=dt.sucursal_id and dt.nro=t.nro
and t.sucursal_id=s.idsucursal and t.fecha_p between '$min'
and '$max' and dt.sucursal_id BETWEEN 2 and 16 GROUP BY s.idsucursal order by t.hora_solicitud asc);;  
"
);
}
else{
