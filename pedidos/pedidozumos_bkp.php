<html>
	<head>
		<title>Pedido Zumos y salsas </title>
...
$min=$_GET["fechaini"];
$max=$_GET["fechamax"];
$query2= $db->GetAll("(select s.idsucursal,p.idproducto,p.nombre as produccion, sum(dt.cantidad)as cantidad , t.hora_solicitud as hora
from detallepedido dt,pedido t, sucursal s,producto p
where p.idcategoria=8 and p.idproducto=dt.producto_id
and t.sucursal_id=dt.sucursal_id and dt.nro=t.nro
and t.sucursal_id=s.idsucursal and t.fecha_p between '$min'
and '$max' GROUP BY p.idproducto order by t.hora_solicitud asc)
UNION
(select s.idsucursal,p.idproducto,p.nombre as produccion, sum(dt.cantidad)as cantidad , t.hora_solicitud as hora
from detallepedido dt,pedido t, sucursal s,producto p
where p.idcategoria=15 and p.idproducto=dt.producto_id
and t.sucursal_id=dt.sucursal_id and dt.nro=t.nro
and t.sucursal_id=s.idsucursal and t.fecha_p between '$min'
and '$max' GROUP BY p.idproducto order by t.hora_solicitud asc)");
$query3= $db->GetAll("(select s.idsucursal,s.nombre as sucursal,p.idproducto,p.nombre as produccion, sum(dt.cantidad)as cantidad, hora_solicitud as hora
from detallepedido dt,pedido t, sucursal s,producto p
where p.idcategoria=15 and p.idproducto=dt.producto_id and dt.producto_id = p.idproducto
and t.sucursal_id=dt.sucursal_id and dt.nro=t.nro
and t.sucursal_id=s.idsucursal and t.fecha_p between '$min'
and '$max' GROUP BY s.idsucursal order by t.hora_solicitud asc)  
union
(select s.idsucursal,s.nombre as sucursal,p.idproducto,p.nombre as produccion, sum(dt.cantidad) as cantidad, hora_solicitud as hora
from detallepedido dt,pedido t, sucursal s,producto p
where p.idcategoria=8 and p.idproducto=dt.producto_id and dt.producto_id = p.idproducto
and t.sucursal_id=dt.sucursal_id and dt.nro=t.nro
and t.sucursal_id=s.idsucursal and t.fecha_p between '$min'
and '$max' and s.idsucursal =10 GROUP BY s.idsucursal  order by t.hora_solicitud asc  )  
union
(select s.idsucursal,s.nombre as sucursal,p.idproducto,p.nombre as produccion, sum(dt.cantidad) as cantidad, hora_solicitud as hora
from detallepedido dt,pedido t, sucursal s,producto p
where p.idcategoria=8 and p.idproducto=dt.producto_id and dt.producto_id = p.idproducto
and t.sucursal_id=dt.sucursal_id and dt.nro=t.nro
and t.sucursal_id=s.idsucursal and t.fecha_p between '$min'
and '$max' and s.idsucursal =8 GROUP BY s.idsucursal  order by t.hora_solicitud asc) 
...
<div class="container">
  ...
  <h2>Programacion de envios de zumos CDP a sucursales</h2>
...
<?php foreach ($query2 as $key) { ?>
  <td><h4><?php echo number_format($key['cantidad'],2); ?></h4></td>
    <?php } ?> 
</tr>
  </table>
  </div>
