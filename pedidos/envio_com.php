...
$min=$_GET["fechaini"];
$max=$_GET["fechamax"];
$query2= $db->GetAll("select s.idsucursal,p.idproducto,p.nombre as produccion, sum(dt.cantidad)as kG , t.hora_solicitud as hora
...
$query3= $db->GetAll("select s.idsucursal,s.nombre as sucursal,p.idproducto,p.nombre as produccion, sum(dt.cantidad)as kG, hora_solicitud as hora
from detallepedido dt,pedido t, sucursal s,producto p
where p.idcategoria=2 and p.idproducto=dt.producto_id
and t.sucursal_id=dt.sucursal_id and dt.nro=t.nro
and t.sucursal_id=s.idsucursal and t.fecha_p between '$min'
