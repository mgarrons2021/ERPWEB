...
$nro=$_POST['nro']; 
$total_eliminacion = $db->GetOne("select sum(subtotal) from detallereciclaje where nro = '$nro' and sucursal_id='$sucur'");
//$inventario = $db->GetOne("SELECT max(nro) FROM inventario where sucursal_id=".$sucur);
$actualizar= $db->Execute("update reciclaje set total='$total_eliminacion' where nro='$nro' and sucursal_id='$sucur'");
if($actualizar!= null) {
