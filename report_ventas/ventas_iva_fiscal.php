...
<table id="example" class="table table-responsive table-striped table-bordered table-hover" cellspacing="" width="100px">
      ...
      <!--  <tfoot>
        ...
        <tbody>
<?php $c=0; $total=0; $sucursal=0;$totaldescuento=0; $descuento=0; $total2=0;foreach ($query as $r){ $c=$c+1; if($r["estado"]!="A"){$total=$total+$r["total"];}else{$total2=$total2+$r["total"];}?>
  <tr class=warning>
      <td><?php echo $c; ?></td>
      <td><?php echo "2"; ?></td>
      ...
      <td><?php echo "0,00";  ?></td>
      <td><?php if($r["nit_ci"] == '0'){echo "SIN NOMBRE"; }else{ echo $r["nombre"]; }?></td>
      ...
      <td><?php echo "0,00";?></td>
      <td><?php echo number_format($r["totaldescuento"],2);?></td>
      <td><?php echo number_format((($r["totaldescuento"]*13)/100),2);?></td>
