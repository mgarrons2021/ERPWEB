...
$min=$_GET["fechaini"];
$max=$_GET["fechamax"];
$query = $db->GetAll("select t.*, u.nombre as usuario, s.nombre as sucursal
from consumoelectrico t inner join usuario u on t.usuario_id = u.idusuario
inner join sucursal s on u.sucursal_id = s.idsucursal
where t.sucursal_id = '$sucur'and t.Fecha between '$min' and '$max'");
}
else{
  $query= $db->GetAll("select t.*, u.nombre as usuario, s.nombre as sucursal
  from consumoelectrico t inner join usuario u on t.usuario_id = u.idusuario
  inner join sucursal s on u.sucursal_id = s.idsucursal
  where t.sucursal_id = '$sucur'");
}
  ?>
